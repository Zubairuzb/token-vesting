...
//                      TEST CONTRACT
// ============================================================
// All test contracts must inherit from Test
// Test function names must start with "test" to be
// recognized and run by Foundry automatically
contract TokenVestingTest is Test {
    ...
    //                         SETUP
    // ============================================================
    // setUp() is a special Foundry function that runs BEFORE
    // every single test. This ensures every test starts from
    // the same clean state — no test can pollute another test.
    // Think of it as resetting everything before each test runs

    function setUp() public {
        // makeAddr() creates a fake address from a label string.
        // The address is deterministic — same string always gives
        // same address. The label also shows in error messages
        // making failing tests much easier to debug
        owner = makeAddr("owner");
        beneficiary = makeAddr("beneficiary");
        stranger = makeAddr("stranger");

        // vm.startPrank(owner) tells Foundry to pretend that
        // ALL transactions from this point come from "owner".
        // Without this, msg.sender would be the test contract itself.
        // vm is Foundry's cheatcode interface — it lets us
        // manipulate blockchain state in ways impossible in production
        vm.startPrank(owner);
    ...
    //                      HELPER FUNCTION
    // ============================================================
    // We will create vesting schedules in many tests.
    // Instead of repeating the same 4 lines every time,
    // this helper function does it in one call.
    ...
    function _createSchedule() internal {
        ...
        // Owner must approve the vesting contract to spend their tokens
        // BEFORE calling createVestingSchedule.
        // This is the standard ERC20 approve + transferFrom pattern:
        // 1. Owner calls approve(spender, amount) on the token
        // 2. Vesting contract calls transferFrom(owner, contract, amount)
    ...
    //                       FUZZ TESTS
    // ============================================================
    // Fuzz tests run your test function hundreds of times with
    // RANDOM inputs generated by Foundry automatically.
    // This catches edge cases you would never think to test manually.
    //
    // Parameters with no fixed value are the fuzz inputs.
    // Foundry generates random values for them each run.
    // We use bound() to constrain random values to valid ranges
